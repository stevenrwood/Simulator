...
// Where littlefs is mounted. Defined HERE rather than at the mount call site because the seeding
// below has to write to the same place - hardcoding "/littlefs" there while this was overridable
// meant the two could disagree silently, and did: an experiment that moved the mount seeded nothing
// at all and looked like the macro CALL still failing.
//
// Root, NOT "/littlefs", and that is deliberate. A real board mounts at /littlefs and this used to
// copy it for fidelity, but this is a LITTLEFS_ENABLE=2 build - which means littlefs AT ROOT, and is
// what provides YModem for ioSender's provisioning. The named-subroutine resolver in ngc_flowctrl.c
// follows that setting: it builds "/littlefs/<name>.macro" only when LITTLEFS_ENABLE == 1, and
// "/<name>.macro" here. Mounting anywhere else means every O<name> CALL fails with
// "error:81 - Unknown flow statement", which names neither paths nor filesystems and cost hours.
// Verified by moving the mount: the CALL stops failing and executes the macro.
//
// ioSender needs no change - it provisions to the mount path the controller REPORTS (AtcMacros ->
// target.Path), so a littlefs@/ mount has it write /tc.macro, exactly where the resolver looks.
#define LITTLEFS_MOUNT_DIR "/"
#endif

// Write ioSender's canonical ATC macros (tc/pcorner/pvisecorner - see embedded_macros.h, generated by
// ioSender/tools/gen-sim-macros.py from ioSender/macros/*.macro, the same files AtcMacros.cs provisions
// onto a real controller) into littlefs on every boot, so a freshly built/launched simulator is
// ATC-macro-ready out of the box - matching a correctly-provisioned real machine - with no runtime
// transfer step required. Unconditional overwrite (cheap - simulated flash, no wear concern) rather
// than a presence/checksum check, so the sim can never drift from whatever ioSender shipped.
//
// Must run, then be followed by a REMOUNT (see the call site): atc_macros_attach - the vfs on_mount
// hook that sets hal.driver_cap.atc and installs the macro-driven M6 handler - fires synchronously
// during fs_littlefs_mount(), before this function gets a chance to write tc.macro. A first mount with
// nothing seeded yet leaves ATC off; writing the macros then remounting re-fires the hook, this time
// finding tc.macro and enabling it. Confirmed empirically (local build, raw $I/$F/M6T1 probe over the
// sim's own telnet port) - without the remount ATC stays off and M6 falls back to a plain manual-pause,
...
static void sim_seed_atc_macros (void)
{
    ...
    // Follow the mount point rather than hardcoding it - see LITTLEFS_MOUNT_DIR above. Handles a
    // mount at "/" without emitting a doubled slash.
    const char *mount = LITTLEFS_MOUNT_DIR;
    const char *sep = (mount[0] && mount[strlen(mount) - 1] == '/') ? "" : "/";

    // Say where the macros are going AND where the core will look for them. These are separately
    // configured and can disagree: the named-subroutine resolver in ngc_flowctrl.c builds
    // "/littlefs/<name>.macro" only when LITTLEFS_ENABLE == 1, and "/<name>.macro" otherwise - so a
    // build that mounts elsewhere will seed macros the resolver can never find, and the only symptom
    // is "error:81 - Unknown flow statement" on an O<name> CALL, which says nothing about paths.
    // This line is here so that is never guessed at again.
#if LITTLEFS_ENABLE == 1
    fprintf(stderr, "seed: mount=%s  O<name> CALL resolves via /littlefs/<name>.macro  [match]\n", mount);
#else
    fprintf(stderr, "seed: mount=%s  O<name> CALL resolves via /<name>.macro (LITTLEFS_ENABLE=%d)%s\n",
            mount, (int)LITTLEFS_ENABLE,
            strcmp(mount, "/") == 0 ? "  [match]" : "  [MISMATCH - named CALL will fail with error:81]");
#endif

    for(size_t i = 0; i < sizeof(macros) / sizeof(macros[0]); i++) {
        char path[64];
        snprintf(path, sizeof(path), "%s%s%s", mount, sep, macros[i].name);

        vfs_file_t *f = vfs_open(path, "w");
        if(f == NULL) {
            fprintf(stderr, "seed: FAILED to open %s for write - it will be absent\n", path);
            continue;
        }

        size_t want = strlen(macros[i].content);
        size_t got = vfs_write(macros[i].content, 1, want, f);
        vfs_close(f);

        if(got != want)
            fprintf(stderr, "seed: SHORT WRITE %s - %u of %u bytes (filesystem full?)\n",
                    path, (unsigned)got, (unsigned)want);
        else
            fprintf(stderr, "seed: %-18s %6u bytes -> %s\n", macros[i].name, (unsigned)want, path);
    }
}
#endif
...
#if LITTLEFS_ENABLE
    // LITTLEFS_MOUNT_DIR is defined next to sim_seed_atc_macros(), which writes to the same place.
    //
    // It mounts at /littlefs (as a real board does) so ioSender's absolute-path ATC provisioning
    // (/littlefs/<name>.macro over YModem) lines up, and atc_macros_attach() keys tc_path off this
    // mount path so M6 and ATC detection follow automatically. NOTE: an earlier version of this
    // comment also claimed the named-subroutine resolver searches /littlefs. It does not in this
    // build - ngc_flowctrl.c only builds that path when LITTLEFS_ENABLE == 1, and this is a
    // LITTLEFS_ENABLE=2 build (which is what provides YModem), so it looks for /<name>.macro
    // instead. The seed logging now states both, and flags the mismatch.
